...
# Paths the pack ships but the GAME owns once it runs. Compared lowercase, forward
# slashes. Deliberately narrow: only add a directory here when the game is KNOWN to
# rewrite it, otherwise real corruption stops being reported.
$DRIFT_DIRS = @("config/ftbquests/")
...
function Locked($rel) {
    ...
    # ...but not all of them use .txt. fzzy_config/keybinds.toml is a real keybind file
    # that the .txt rule alone would sweep, so match on the name too. Deliberately not
    # widened to every client setting - that would defeat rung 3 entirely.
    if ($p.StartsWith("config/") -and ($p -match "keybind|keymap")) { return $true }
    return $false
}
...
function Diagnose-Inst($dir) {
    ...
    $d = [PSCustomObject]@{
        ...
        Tracked = 0; Jars = 0
        Orphans = @(); Corrupt = @(); Missing = @(); Drifted = @(); Unchecked = 0
    ...
    if ($pw) {
        ...
        foreach ($p in $props) {
            ...
            if ($got -ne "$($want.value)".ToLower()) {
                # A third class between clean and corrupt: files the pack ships but the
                # game rewrites at runtime. FTB Quests normalises every
                # config/ftbquests/quests/*.snbt on first world load, so a played
                # instance drifts from the recorded hash permanently - 12 files here.
                # Counting those as corrupt made the tool report 12 problems forever and
                # re-fetch the same files on every run, which is the "repair tool invents
                # problems" failure the spec exists to prevent.
                # Rungs 2 and 3 still restore them by dropping packwiz.json, so nothing
                # is lost by leaving them alone.
                if ($DRIFT_DIRS | Where-Object { $loc.Replace("\","/").ToLower().StartsWith($_) }) {
                    $d.Drifted += $loc
                } else {
                    $d.Corrupt += $loc
                }
...
function Problem-Count($d) {
    ...
    # Being behind the published pack is the normal state between a pack update and
    # the player's next launch: the packwiz PreLaunchCommand fixes it automatically.
    # Counting it turned a healthy install into "Found 1 problem(s)" in red and offered
    # a repair that moves zero files. It stays in the report, it just is not a problem.
...
$RUNG_DESC = @("",
 ...
 "Moves the whole mods and config folders into quarantine and lets the updater rebuild them. Your world, screenshots and shaders stay. Mod settings and keybinds inside config are moved too - the updater will not put those back, but nothing is deleted and it is all in the quarantine folder.")
...
function Repair-Run($rung) {
    ...
    $btn.BackColor = $C_ACC
    Mark 1 "now"
    # Name the instances. An instance is claimed on the strength of a packwiz.json
    # alone, so a player who also has another packwiz modpack installed gets it swept
    # too - and this is the last screen before rung 3 touches every child of mods/ and
    # config/. Saying which copies are affected is the minimum.
    $names = (@($script:Diag | ForEach-Object { $_.Name }) -join ", ")
    Card ("Running the " + $RUNG_NAME[$rung]) ($RUNG_DESC[$rung] + "`r`n`r`nThis will run on: $names.`r`n`r`nNothing is deleted. Everything moved keeps its original path inside a quarantine folder, so any of it can be put back by hand.")
    $bar.Visible = $true
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    Say ""
    ...
    foreach ($d in $script:Diag) {
        ...
        if ($rung -le 2) {
            ...
            foreach ($rel in (@($d.Orphans) + @($d.Corrupt))) {
                # Only mods/ and config/ are in scope for the targeted rungs. The pack
                # also tracks resourcepacks/*.zip and shaderpacks/*.zip, and the spec
                # lists both as untouched by ANY rung - the closing copy promises the
                # player their shaders were left alone.
                $top = $rel.Replace("\","/").Split("/")[0].ToLower()
                if ($top -ne "mods" -and $top -ne "config") {
                    Say "  failed its checksum but left in place, outside mods and config: $rel"
                    continue
                }
    ...
    $script:OnPrimary = { $form.Close() }
    if ($script:Rung -le 3) {
        $script:OnSecondary = { Repair-Again $script:Rung }
        Buttons "Close" ("Run the " + $RUNG_NAME[$script:Rung])
    } else {
        Buttons "Close" $null
    }
