...
diagnose_instance() {   # diagnose_instance <instance dir> <scratch dir>
    ...
    if [ ! -f "$mcdir/packwiz.json" ]; then
        ...
        # like an orphan.
        # The guard the comment above describes was missing. A packwiz.json that
        # PARSES but yields no file entries - "cachedFiles": {}, an all-server-side
        # pack, or a future key rename - leaves loc.txt empty, and comm then reports
        # every jar in mods/ as an orphan. On a real install that is all 146 mods
        # swept by the gentlest rung, the one players are told to start with.
        if [ "$D_TRACKED" -gt 0 ]; then
            LC_ALL=C sort -u "$s/loc.txt" > "$s/loc.sorted"
            LC_ALL=C comm -23 "$s/jars.txt" "$s/loc.sorted" > "$s/orphan.txt"
        else
            printf 'packwiz.json records no files, so leftover jars were not checked\n' \
                >> "$s/note.txt"
        fi
...
quarantine() {   # quarantine <mcdir> <relative path>
    ...
    case "$rel" in
        *..*|"$QUAR"|"$QUAR"/*|saves|saves/*|screenshots|screenshots/*|\
        ...
        config/*keybind*|config/*keymap*)
            printf 'left alone, yours not the pack: %s\n' "$rel" >> "$CHANGED"
            return 0 ;;
    esac
    dest="$mcdir/$QUAR/$STAMP/$rel"
    mkdir -p "$(dirname "$dest")" 2>/dev/null || return 1
    # If dest already exists as a directory, mv puts the source INSIDE it rather than
    # at it, producing config/ftbquests/ftbquests/... . Nothing is lost, but the report
    # tells the player they can drag it back, and that path would not restore correctly.
    if [ -e "$dest" ]; then
        n=1
...
run_rung() {   # run_rung <instance dir> <scratch dir> <1|2|3>
    ...
    # Targeted rungs act only on mods/ and config/. The pack also tracks
    # resourcepacks/*.zip and shaderpacks/*.zip, and the closing copy promises the
    # player their shaders were left as they were.
    in_scope() { case "$1" in mods/*|config/*) return 0 ;; *) return 1 ;; esac; }
    while IFS= read -r rel; do
        in_scope "$rel" && quarantine "$mcdir" "$rel"
    done < "$s/orphan.txt"
    while IFS= read -r rel; do
        if in_scope "$rel"; then quarantine "$mcdir" "$rel"
        else printf 'failed its check but left in place, outside mods and config: %s\n' \
                 "$rel" >> "$CHANGED"
        fi
    done < "$s/corrupt.txt"
...
Start with the smallest one that could work." \
    ...
    # Read the answer back rather than trusting the exit status. AppleScript raises
    # error -128 only for a button it recognises as the cancel one, and it decides that
    # from the name "Cancel" alone - so "Quit" exits 0 and, with the output discarded,
    # the repair would proceed anyway. This is the only gate in front of option 3, which
    # sweeps every child of mods/ and config/.
    case "$(osascript -e "display dialog \"Last check before anything moves.
...
default button \"Do it\" with title \"$TITLE\"" 2>/dev/null)" in
        *"Do it"*) : ;;
        *) stop_here ;;
    esac
